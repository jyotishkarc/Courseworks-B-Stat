...
state.total <-list()
H <- data.frame()
age.all.v1 <- age.all.rv <- c()

for(k in 1:nrow(temp.urban.v1)){
   age.all.v1[k] <- temp.urban.v1[k,42:44] %>% paste0() %>% 
                     stringr::str_c(collapse = "") %>% as.numeric()
}

for(k in 1:nrow(temp.urban.rv)){
   age.all.rv[k] <- temp.urban.rv[k,42:44] %>% paste0() %>% 
      stringr::str_c(collapse = "") %>% as.numeric()
}

beep()

age.class.1.v1 <- which(age.all.v1 >= 15 & age.all.v1 <= 29)
age.class.2.v1 <- which(age.all.v1 >= 15)
age.class.3.v1 <- which(age.all.v1 >= 0)

age.class.1.rv <- which(age.all.rv >= 15 & age.all.rv <= 29)
age.class.2.rv <- which(age.all.rv >= 15)
age.class.3.rv <- which(age.all.rv >= 0)

for(num in setdiff(19:36, 31)){

   state <- state.codes.char[num] %>% strsplit(split = "") %>% unlist()
   cat("\n\n",state.codes.char[num],state.codes$State[num],"\n\n")
   
   temp.urban.v1.state <- temp.urban.v1[age.class.3.v1, ] %>% 
                              filter(V13 == state[1] & V14 == state[2])
   temp.urban.rv.state <- temp.urban.rv[age.class.3.rv, ] %>% 
                              filter(V13 == state[1] & V14 == state[2])
